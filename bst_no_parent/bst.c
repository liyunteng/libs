 ...
 * License: GPL
 * Update time:  2016/04/20 07:59:28
 *
...
struct bst_node *bst_insert(struct bst_node **root, datatype data)
{
	if (!*root) {
...
struct bst_node *bst_create(struct bst_node **root, datatype a[], int len)
{
	for (int i = 0; i < len; i++) {
		bst_insert(root, a[i]);
	}
...
void bst_pre_iterate(struct bst_node *root)
{
	if (root) {
		printf("%d ", root->data);
...
void bst_in_iterate(struct bst_node *root)
{
	if (root) {
...
void bst_post_iterate(struct bst_node *root)
{
	if (root) {
...
struct bst_node *bst_search(struct bst_node *root, datatype key)
{
	if (!root)
		return NULL;
...
int bst_delete(struct bst_node **root, datatype key)
{
	if (!*root)
		return -1;
