...
typedef enum { AVL_TREE_LT, AVL_TREE_EQ, AVL_TREE_GT, AVL_TREE_INVALID } avl_tree_compare_code_td;
 ...
 */
typedef enum { AVL_FREE_OBJECTS, AVL_DONT_FREE_OBJECTS } avl_tree_shutdown_code_td;
...
typedef avl_tree_compare_code_td (*avl_tree_compare_fn)(void *obj_one, void *obj_two);
...
int avl_tree_init_v2(avl_tree_compare_fn compare_fn, avl_tree_malloc_fn malloc_fn,
...
int avl_tree_init_fns(avl_tree_malloc_fn malloc_fn, avl_tree_free_fn free_fn,
                      avl_tree_cleanup_fn cleanup_fn, avl_tree_object_sanity_fn snity_fn,
                      avl_tree_h_td avl_tree);

int avl_tree_allocate_object(avl_tree_h_td avl_tree, void **object, uint32_t size);

int avl_tree_free_object(avl_tree_h_td avl_tree, void *object);

int avl_tree_insert(avl_tree_h_td avl_tree, void *object, void **existing_object);

int avl_tree_delete(avl_tree_h_td avl_tree, void *object, void **deleted_object);
...
int avl_tree_search(avl_tree_h_td avl_tree, avl_tree_search_options_td options,
                    void *compare_object, void **found_object, avl_tree_search_options_td *result);
int avl_tree_walk(avl_tree_h_td avl_tree, avl_tree_walk_fn walk_fn, void *ctx,
                  avl_tree_walk_code_td *result);

int avl_tree_walk_safe(avl_tree_h_td avl_tree, avl_tree_walk_fn walk_fn, void *ctx,
                       avl_tree_walk_code_td *result);

int avl_tree_shutdown(avl_tree_h_td *avl_tree, avl_tree_shutdown_code_td shutdown_code);
