...
#define SHA384_512AddLength(context, length)                                   \
    (addTemp[3] = (length),                                                    \
     ...
     (context)->Corrupted =                                                    \
         (((context)->Length[3] < (length)) && ((context)->Length[2] == 0)     \
          && ((context)->Length[1] == 0) && ((context)->Length[0] == 0)) ?     \
             shaInputTooLong :                                                 \
             (context)->Corrupted)
...
/* Initial Hash Values: FIPS 180-3 sections 5.3.4 and 5.3.5 */
static uint64_t SHA384_H0[] = {0xCBBB9D5DC1059ED8ull, 0x629A292A367CD507ull,
                               0x9159015A3070DD17ull, 0x152FECD8F70E5939ull,
                               0x67332667FFC00B31ull, 0x8EB44A8768581511ull,
                               0xDB0C2E0D64F98FA7ull, 0x47B5481DBEFA4FA4ull};
static uint64_t SHA512_H0[] = {0x6A09E667F3BCC908ull, 0xBB67AE8584CAA73Bull,
...
#ifdef USE_32BIT_ONLY
static int
SHA384_512Reset(SHA512Context *context, uint32_t H0[SHA512HashSize / 4])
#else  /* !USE_32BIT_ONLY */
static int
SHA384_512Reset(SHA512Context *context, uint64_t H0[SHA512HashSize / 8])
#endif /* USE_32BIT_ONLY */
{
    int i;
    if (!context)
...
#ifdef USE_32BIT_ONLY
    context->Length[0] = context->Length[1] = context->Length[2] =
        context->Length[3]                  = 0;
...
SHA384_512ProcessMessageBlock(SHA512Context *context)
{
    ...
    uint32_t temp1[2], temp2[2], temp3[2], temp4[2],
        temp5[2];       /* Temporary word values */
