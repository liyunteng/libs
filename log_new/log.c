...
log_rename_logfile(log_ctx_p ctx)
{
    ...
    snprintf(new_file_name, len,"%s.%lu", old_file_name, ctx->file_idx);
...
log_map_file(log_ctx_t *log_ctx, int window_size_factor)
{
    ...
    if (log_ctx->mmap_window.offset == 0) {
        memset(&log_ctx->mmap_window, 0, sizeof(log_ctx->mmap_window));

        page_size = getpagesize();
        mmap_window_size = (4*1024*1024 * window_size_factor + page_size - 1) &(~(page_size-1));
        if (mmap_window_size > 4*1024*1024) {
            return -1;
        }

        if (mmap_window_size > file_size) {
            log_ctx->file_size = ((mmap_window_size+1024 -1) & (~(1024-1)))/1024;
        ...
        log_ctx->mmap_window.addr = mmap(0, mmap_window_size, PROT_READ|PROT_WRITE, MAP_SHARED, fd, 0);
        ...
        len = log_ctx->mmap_window.offset - (log_ctx->mmap_window.len - log_ctx->mmap_window.data_offset);

        tail = log_ctx->mmap_window.addr + log_ctx->mmap_window.data_offset;
        for (c=tail-1; c >= (char *)log_ctx->mmap_window.addr; --c) {
            if (*c == '\n') {
                break;
            }
        }
        ...
        if (size) {
            ...
            if (!str) {
                ...
                if (log_ctx->mmap_window.data_offset < log_ctx->mmap_window.len) {
        ...
        if (str) {
            if (size <= log_ctx->mmap_window.len) {
                strncpy(log_ctx->mmap_window.addr, str, log_ctx->mmap_window.len);
        ...
        if (log_ctx->mmap_window.data_offset < log_ctx->mmap_window.len) {
            len = log_ctx->mmap_window.len - log_ctx->mmap_window.data_offset;
            len = (len + page_size-1) &(~(page_size-1));
            log_ctx->mmap_window.offset -= len;
            if (log_ctx->mmap_window.offset < 0) {
                log_ctx->mmap_window.offset = 0;
            }
            log_ctx->mmap_window.data_offset -= log_ctx->mmap_window.len - len;
            ...
            log_ctx->mmap_window.addr = mmap(0, mmap_window_size, PROT_READ|PROT_WRITE, MAP_SHARED,
                                             log_ctx->mmap_window.fd, log_ctx->mmap_window.offset);
            if (log_ctx->mmap_window.addr == MAP_FAILED) {
                close(log_ctx->mmap_window.fd);
                return -1;
            }
            ...
            memset(&log_ctx->mmap_window.msync_time, 0, sizeof(log_ctx->mmap_window.msync_time));
        } else {
            len = file_size - log_ctx->mmap_window.offset;
            if (mmap_window_size > len) {
                mmap_window_size = len;
...
vlog_print(log_ctx_p ctx, const char *fmt, va_list ap)
{
    ...
    ret = vsnprintf(ctx->mmap_window.addr + ctx->mmap_window.data_offset,
                    len, fmt, ap);
    if (ret < 0) {
        ret = errno;
        goto err_quit;
    ...
    if (ctx) {
        ...
        if(ctx->mmap_window.addr) {
            ...
            ftruncate(ctx->mmap_window.fd,
                      ctx->mmap_window.offset - (ctx->mmap_window.len - ctx->mmap_window.data_offset));
...
int
log_init_ex(const char *log_name, const char *file_path, const uint16_t files,
            const uint32_t file_size, log_ctx_p *out_ctx)
{
    log_ctx_t *log_ctx;
    char filename[256];
    ...
    *out_ctx = NULL;
    ...
    printf("num_files: %d file_size: %d\n", log_ctx->num_files, log_ctx->file_size);
    if ((ret = log_file_name(filename, sizeof(filename), log_ctx)) != 0) {
        return -1;
    }
...
log_ctx_p
log_init(const char *log_name, const char *file_path, const uint8_t files,
