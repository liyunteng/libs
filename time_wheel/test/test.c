...
static void timer_check_cascade()
{
	...
	timers = (twtimer_t*)calloc(TIMER, sizeof(twtimer_t));
...
static void timer_simple_test()
{
    int i, j;
    uint64_t now;
    time_wheel_t *wheel = NULL;
    twtimer_t *timers = NULL;
    int s = 0;
    ...
    timers = (twtimer_t *)calloc(N, sizeof(twtimer_t));
    assert(timers);
    for (i = 0; i < N; i++) {
        timers[i].ontimeout = ontimeout;
        timers[i].param = &timers[i];
