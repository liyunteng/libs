...
struct time_wheel *time_wheel_create(uint64_t clock)
{
...
int time_wheel_destroy(struct time_wheel *tm)
{
    assert(0 == tm->count);
...
int twtimer_start(struct time_wheel *tm, struct twtimer *timer)
{
    int r;
...
int twtimer_stop(struct time_wheel *tm, struct twtimer *timer)
{
...
int twtimer_process(struct time_wheel *tm, uint64_t clock)
{
    int index;
...
static int twtimer_cascade(struct time_wheel *tm, struct time_bucket *tv, int index)
{
    struct twtimer *timer;
    struct twtimer *next;
    next = tv[index].first;
