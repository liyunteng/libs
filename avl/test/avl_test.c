...
void *my_malloc_fn(size_t size)
{
    return (malloc(size));
}

void my_free_fn(void *mem_ptr)
{
    free(mem_ptr);
}

void my_cleanup_fn(void *mem_ptr)
{
...
my_compare_fn(void *obj1, void *obj2)
{
    ...
    int compare;
    compare = strcmp(((my_object_t *)obj1)->key,
                     ((my_object_t *)obj2)->key);
...
int main(int argc, char *argv[])
{
    ...
    if (argc != 2) {
        ...
        rc = avl_tree_init(my_compare_fn,
                           AVL_TREE_OPTION_DEFAULT,
                           &my_avl_tree);
        if (rc == 0) {
            showtime = time(NULL);
        ...
        if (rc == 0) {
            rc = avl_tree_init_fns(my_malloc_fn,
                                   my_free_fn,
                                   my_cleanup_fn,
                                   my_object_sanity_fn,
                                   my_avl_tree);
        ...
        if (rc == 0) {
            rc = avl_tree_allocate_object(my_avl_tree,
                                          (void *)&obj,
                                          sizeof(my_object_t));
        }
        if (rc == 0) {
        ...
        if (rc == 0) {
            rc = avl_tree_insert(my_avl_tree,
                                 obj,
                                 (void *)&found_obj);
            if (found_obj) {
                printf("key = %s\n", found_obj->key);
        ...
        if (rc == 0) {
            ...
            rc = avl_tree_search(my_avl_tree,
                                 AVL_TREE_OPTION_EQ,
                                 obj,
                                 (void *)&found_obj,
                                 &search_result);
