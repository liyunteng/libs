...
static inline int mpool_empty(mpool_ctx_t *ctx)
{
    return ctx->tail == ctx->head;
}

static inline int mpool_full(mpool_ctx_t *ctx)
{
    return ctx->tail - ctx->head >= ctx->max;
}

static inline int mpool_size(mpool_ctx_t *ctx)
{
    return ctx->max;
}

static inline int mpool_count(mpool_ctx_t *ctx)
{
