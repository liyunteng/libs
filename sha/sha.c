 ...
 */
int
USHAReset(USHAContext *context, enum SHAversion whichSha)
{
    if (!context)
        return shaNull;
 ...
 */
int
USHAInput(USHAContext *context, const uint8_t *bytes, unsigned int bytecount)
{
    if (!context)
        return shaNull;
 ...
 */
int
USHAFinalBits(USHAContext *context, uint8_t bits, unsigned int bit_count)
{
    if (!context)
        return shaNull;
 ...
 */
int
USHAResult(USHAContext *context, uint8_t Message_Digest[USHAMaxHashSize])
{
    if (!context)
        return shaNull;
 ...
 */
const char *
USHAHashName(enum SHAversion whichSha)
{
    switch (whichSha) {
    case SHA1:
        return "SHA1";
