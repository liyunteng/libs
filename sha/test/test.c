...
SHA1, 80,
     ...
     "\xE8\xE9\xEA\xEB\xEC\xED\xEE\xEF\xF0\xF1\xF2\xF3\xF4\xF5"
     "\xF6\xF7\xF8\xF9\xFA\xFB\xFC\xFD\xFE\xFF",
     20, "8ADAE09A2A307059478D309B26C4115A224CFAF6", 82,
    ...
    {/* RFC 5869 A.6. Test Case 6 */
     ...
     "\x0b\x0b\x0b\x0b\x0b\x0b\x0b\x0b",
     0, "", 0, "", 20, "DA8C8A73C7FA77288EC6F5E7C297786AA0D32D01", 42,
    ...
    {/* RFC 5869 A.7. Test Case 7. */
     ...
     0, 0, 0, "", 20, "2ADCCADA18779E7C2077AD2EB19D3F3E731385DD", 42,
...
int
checkmatch(const unsigned char *hashvalue, const char *hexstr, int hashsize)
{
    int i;
 ...
 */
void
printstr(const char *str, int len)
{
    for (; len-- > 0; str++)
 ...
 */
void
printxstr(const char *str, int len)
{
    char *sep = "";
    for (; len-- > 0; str++) {
 ...
 */
void
printResult(uint8_t *Message_Digest, int hashsize, const char *hashname,
            const char *testtype, const char *testname, const char *resultarray,
            int printResults, int printPassFail)
{
    ...
    if (printResults == PRINTTEXT) {
        ...
        for (i = 0; i < hashsize; i += 3) {
            ...
            if (i + 1 < hashsize)
                b |= Message_Digest[i + 1] >> 4;
            putchar(sm[b]);
            if (i + 1 < hashsize) {
                b = (Message_Digest[i + 1] & 0x0f) << 2;
                if (i + 2 < hashsize)
                    b |= Message_Digest[i + 2] >> 6;
                putchar(sm[b]);
            } else
                putchar('=');
            if (i + 2 < hashsize)
                putchar(sm[Message_Digest[i + 2] & 0x3f]);
            else
                putchar('=');
        }
        putchar('\n');
 ...
 */
int
hash(int testno, int loopno, int hashno, const char *testarray, int length,
     ...
     int printResults, int printPassFail)
{
    ...
    err = info ? hkdfReset(&hkdf, hashes[hashno].whichSha, keyarray, keylen) :
          keyarray ?
                 hmacReset(&hmac, hashes[hashno].whichSha, keyarray, keylen) :
    ...
    err = info ? hkdfResult(&hkdf, 0, info, infolen, Message_Digest, okmlen) :
          keyarray ? hmacResult(&hmac, Message_Digest) :
                     USHAResult(&sha, Message_Digest);
    if (err != shaSuccess) {
        fprintf(stderr,
                "hash(): %s Result Error %d, could not compute "
                "message digest.\n",
                info     ? "hkdf" :
                keyarray ? "hmac" :
                           "sha",
    ...
    printResult(Message_Digest, info ? okmlen : hashsize, hashes[hashno].name,
                info     ? "hkdf standard test" :
                keyarray ? "hmac standard test" :
                           "sha standard test",
                buf, resultarray, printResults, printPassFail);
    return err;
}
 ...
 */
int
hashHkdf(int testno, int loopno, int hashno, int printResults,
         int printPassFail)
{
    int err;
    ...
    if (printResults == PRINTTEXT) {
        ...
        printxstr(hkdfhashes[testno].saltarray, hkdfhashes[testno].saltlength);
        printf("'\n\tIKM\t'");
        printxstr(hkdfhashes[testno].ikmarray, hkdfhashes[testno].ikmlength);
        printf("'\n\tINFO\t'");
        printxstr(hkdfhashes[testno].infoarray, hkdfhashes[testno].infolength);
        printf("'\n");
 ...
 */
int
hashfile(int hashno, const char *hashfilename, int bits, int bitcount,
         ...
         int printPassFail)
{
    ...
    printResult(Message_Digest, info ? okmlen : hashsize, hashes[hashno].name,
                "file", hashfilename, resultarray, printResults, printPassFail);
    if (hashfp != stdin)
        fclose(hashfp);
    if (info)
        free(Message_Digest);
 ...
 */
void
randomtest(int hashno, const char *seed, int hashsize,
 ...
 */
int
findhash(const char *argv0, const char *opt)
{
    int i;
 ...
 */
void
testErrors(int hashnolow, int hashnohigh, int printResults, int printPassFail)
{
    USHAContext usha;
...
sha_test(int argc, char **argv)
{
    ...
    if ((testnolow < 0)
        ...
        || (info && ((infolen <= 0) || (okmlen <= 0))) || (info && !hmacKey))
        usage(argv[0]);
    /*
        ...
        for (loopno = 1; (loopno <= loopnohigh) && (err == shaSuccess);
            ...
            if (hashstr)
                ...
                for (testno = testnolow;
                     (testno <= testnohigh) && (err == shaSuccess);
                     ++testno) {
                    if (runHmacTests) {
                        err = hash(
                            ...
                            0, 0, 0, hmachashes[testno].resultarray[hashno],
                            hmachashes[testno].resultlength[hashno],
                            printResults, printPassFail);
                    } else if (runHkdfTests) {
                        err = hashHkdf(testno, loopno, hashno, printResults,
                                       printPassFail);
                    } else { /* sha tests */
 ...
 */
int
scasecmp(const char *s1, const char *s2)
{
    for (;;) {
...
sha_test1(void)
{
    ...
    input_t sha1_input[] = {
        ...
        {"abcdbcdecdefdefgefghfghighijhijkijkljklmklmnlmnomnopnopq",
         "84983E441C3BD26EBAAE4AA1F95129E5E54670F1"},
        {"hijklmghijklmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrst"
         "u",
         "68FC37C932379BFD9724AEE246144B560C88C179"},
        {"\x49\xb2\xae\xc2\x59\x4b\xbe\x3a\x3b\x11\x75\x42\xd9\x4a\xc8",
         "AC07070D9D32211A3DFED9A489129B6736884013"},
        {"\x9a\x7d\xfd\xf1\xec\xea\xd0\x6e\xd6\x46\xaa\x55\xfe\x75\x71\x46",
         "82ABFF6605DBE1C17DEF12A394FA22A82B544A35"},
    };
    int ret, i, j, n;
    uint8_t sha1_result[SHA1HashSize] = {0};
    char buf[256] = {0};
    int len;

    for (i = 0; i < sizeof(sha1_input) / sizeof(sha1_input[0]); i++) {
        ret = SHA1Reset(&sha1ctx);
        assert(ret == 0);
        SHA1Input(&sha1ctx, (uint8_t *)sha1_input[i].input, strlen(sha1_input[i].input));
        SHA1FinalBits(&sha1ctx, 0x00, 0);
        len = SHA1Result(&sha1ctx, sha1_result);

        n = 0;
        memset(buf, 0x00, 256);
        for (j = 0; j < SHA1HashSize; j++) {
            n += snprintf(buf+n, 255-n, "%02X", sha1_result[j]);
        }
        if (memcmp(buf, sha1_input[i].output, len) == 0) {
            printf("%s: %s PASS\n", sha1_input[i].input, sha1_input[i].output);
        } else {
            printf("%s: %s FAILED %s\n", sha1_input[i].input, sha1_input[i].output, buf);
        }
    }
}
...
sha_test2(void)
{
    ...
    int ret, i, j, n;
    uint8_t result[USHAMaxHashSize] = {0};
    char buf[256] = {0};
    int len;

    for (i = 0; i < sizeof(input) / sizeof(input[0]); i++) {
        ret = USHAReset(&ctx, SHA512);
        assert(ret == 0);
        USHAInput(&ctx, (uint8_t *)input[i].input, strlen(input[i].input));
        USHAFinalBits(&ctx, 0x00, 0);
        len = USHAResult(&ctx, result);

        n = 0;
        memset(buf, 0x00, 256);
        for (j = 0; j < len; j++) {
            n += snprintf(buf+n, 255-n, "%02X", result[j]);
        }
        if (memcmp(buf, input[i].output, len) == 0) {
            printf("%s: %s PASS\n", input[i].input, input[i].output);
        } else {
            printf("%s: %s FAILED %s\n", input[i].input, input[i].output, buf);
        }
    }
}
...
sha_test3(void)
{
    ...
    for (i = 0; i < sizeof(input) / sizeof(input[0]); i++) {
        ret = hmacReset(&ctx, SHA1, key, 8);
        assert(ret == 0);
        hmacInput(&ctx, (uint8_t *)input[i].input, strlen(input[i].input));
        hmacFinalBits(&ctx, 0x00, 0);
        len = hmacResult(&ctx, result);
        ...
        if (memcmp(buf, input[i].output, n) == 0) {
            printf("%s: %s PASS\n", input[i].input, input[i].output);
        } else {
            printf("%s: %s FAILED %s\n", input[i].input, input[i].output, buf);
        }
