...
static inline const char* ts_type(int type)
{
	switch (type)
	{
...
{
    int i;
    static int streams[512] = {0};
    for (i = 0; i < 512; i++) {
        if (streams[i] == codecid) {
            return i;
        }
    }

    i = mpeg_ts_add_stream(ts, codecid, NULL, 0);
    streams[i] = codecid;
    printf("new i == %d\n", i);
    return i;
}
...
{
	printf("[%s] pts: %08lu, dts: %08lu program: %d stream: %d avtype: %d bytes: %lu%s\n", ts_type(avtype), (unsigned long)pts, (unsigned long)dts,
           program, stream, avtype, bytes,
           flags ? " [I]":"");
